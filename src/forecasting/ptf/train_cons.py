...
# PATHS / CONFIG
# =========================================================
FEATURES_PATH = Path("data/features/consumption/consumption_features.parquet")

ARTIFACTS_DIR = Path("artifacts/modelling_artifact_consumption_lgbm")
...
EXPERIMENT_NAME = "consumption_forecasting_lgbm"
...
# FEATURES
# =========================================================
def build_consumption_feature_list(
    df: pd.DataFrame,
    target_base_col: str,
) -> list[str]:
    time_features = [
...
# =========================================================
def main() -> None:
    print("Loading consumption feature dataset...")
    ...
    print(f"Configured consumption targets: {TARGET_COLUMNS}")
    ...
    for target_base_col in TARGET_COLUMNS:
        ...
        feature_cols = build_consumption_feature_list(df_target, target_base_col=target_base_col)
        ...
        model_version = f"{target_base_col}_consumption_lgbm_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

        with mlflow.start_run(run_name=f"lightgbm_final_train_val_{target_base_col}"):
            mlflow.set_tag("model_version", model_version)
            mlflow.set_tag("pipeline", "consumption")
            ...
            lgbm_metrics = {
                "model": "lightgbm_final_train_val_consumption",
    ...
    if not all_results:
        raise ValueError("No consumption targets were trained successfully.")

    summary_df = pd.DataFrame(all_results)
    summary_df.to_csv(ARTIFACTS_DIR / "all_targets_summary.csv", index=False)

    print("\nConsumption training completed for all available targets.")
    print(summary_df)
