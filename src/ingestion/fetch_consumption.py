...
OUT_DIR = "data/raw/epias/consumption"
...
def fetch_consumption_raw(start_date: str, end_date: str, tgt: str) -> dict:
    headers = {
        "TGT": tgt,
    ...
    r = requests.post(CONSUMPTION_URL, headers=headers, json=payload, timeout=60)

    if r.status_code != 200:
        print("CONSUMPTION status:", r.status_code)
        print("CONSUMPTION text head:", r.text[:500])
...
def fetch_single_chunk(
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            raw = fetch_consumption_raw(start_date, end_date, tgt)
