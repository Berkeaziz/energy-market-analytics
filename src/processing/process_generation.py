...
RAW_PATH = Path("data/raw/epias/generation")
PROCESSED_PATH = Path("data/processed/generation/generation_processed.parquet")
...
def resolve_duplicate_timestamps(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["_gen_positive_priority"] = (df["generation"] > 0).astype("int8")
    df["_gen_sort_value"] = df["generation"].fillna(float("-inf"))

    df = df.sort_values(
        by=["date", "_gen_positive_priority", "_gen_sort_value"],
        ascending=[True, False, False],
        kind="mergesort",
    )

    df = df.drop_duplicates(subset=["date"], keep="first").copy()
    df = df.drop(columns=["_gen_positive_priority", "_gen_sort_value"])
    return df
...
def clean_epias_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    exclude_cols = {"date", "hour", "_chunk_start", "_chunk_end", "year", "month"}
    numeric_cols = [col for col in df.columns if col not in exclude_cols]

    for col in numeric_cols:
        df[col] = pd.to_numeric(df[col], errors="coerce")

    df = df.dropna(subset=["date", "generation"]).copy()
